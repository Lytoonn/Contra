...
EXTERNAL_API_MAX_LEN = 2000
EXTERNAL_STYLE_MAX_LEN = 2000

class PlanChoice(models.Model):
    plan_code = models.CharField(
        max_length = 2, unique = True, blank = False, verbose_name = _t('Plan code')
    )
    name = models.CharField(
        max_length = PLAN_CHOICE_NAME_MAX_LEN, unique = True, blank = False, verbose_name= _t('Plan name')
    )
    cost = models.DecimalField(
        max_digits = SUBSCRIPTION_COST_MAX_DIGITS, decimal_places = 2, verbose_name = _t('Plan cost')
    )
    ...
    external_api_url = models.CharField(
        max_length = EXTERNAL_API_MAX_LEN,unique = True, verbose_name = _t('External API URL')
    )
...
class Subscription(models.Model):
    ...
    def __str__(self) -> str:
        ...
        return f'{self.user.firstName} {self.user.lastName}: {plan_choice.name} {_t2("subscription")}'
